...
<tr><td class="sym"><img src="images/m5-sym-terminal.svg"></td><td><b>Terminal</b> — จุดเริ่มต้น หรือจุดสิ้นสุดของโปรแกรม</td></tr>
<tr><td class="sym"><img src="images/m5-sym-process.svg"></td><td><b>Process</b> — การประมวลผลทั่วไป เช่น การคำนวณ หรือการกำหนดค่า</td></tr>
<tr><td class="sym"><img src="images/m5-sym-io.svg"></td><td><b>Input/Output</b> — การรับค่าเข้าสู่โปรแกรม หรือการแสดงผลลัพธ์</td></tr>
<tr><td class="sym"><img src="images/m5-sym-decision.svg"></td><td><b>Decision</b> — คำสั่งที่ต้องตัดสินใจ มีทางเลือกสองทาง (True / False)</td></tr>
<tr><td class="sym"><img src="images/m5-sym-connector.svg"></td><td><b>Connector</b> — แสดงจุดเชื่อมของโปรแกรม</td></tr>
</table>
...
<table class="symbol-table">
<tr><th>Symbol</th><th>Meaning</th></tr>
<tr><td class="sym"><img src="images/m5-sym-link.svg"></td><td><b>Link</b> — เชื่อมต่อระหว่างส่วนของโปรแกรม (เช่น ข้ามหน้ากระดาษ)</td></tr>
<tr><td class="sym"><img src="images/m5-sym-function.svg"></td><td><b>Function</b> — ส่วนของ Function ที่เรียกใช้ (รายละเอียดอยู่ที่อื่น)</td></tr>
<tr><td class="sym"><img src="images/m5-sym-flowline.svg"></td><td><b>Flow line</b> — ทิศทางการทำงาน เรียงจากบนลงล่าง</td></tr>
<tr><td class="sym"><img src="images/m5-sym-loop.svg"></td><td><b>Loop arrow</b> — ลูกศรล่างขึ้นบน แสดงการทำซ้ำ/วนลูป</td></tr>
</table>
...
:::
::: {.column width="50%"}
![](images/m5-example1.svg){height="420px" fig-align="center" .fragment}
:::
::::
...
:::: {.columns}
::: {.column width="30%"}
![](images/m5-if.svg){height="480px" fig-align="center" .fragment}
:::
::: {.column width="70%"}
::: {.fragment}
<pre class="code-pattern" style="font-size:1.1em"><span class="kw">if</span> <span class="cond">condition</span>:
   <span class="stmt">statement</span>
<span style="color:#5d6d7e">next-statement</span></pre>
:::
...
:::: {.columns}
::: {.column width="30%"}
![](images/m5-if-compound.svg){height="480px" fig-align="center" .fragment}
:::
::: {.column width="70%"}
::: {.fragment}
<pre class="code-pattern" style="font-size:1.1em"><span class="kw">if</span> <span class="cond">condition</span>:
   <span class="stmt">statement1</span>   <span class="cmt">┐</span>
   <span class="stmt">statement2</span>   <span class="cmt">├ Compound statement</span>
   <span class="stmt">...</span>          <span class="cmt">┘</span>
<span style="color:#5d6d7e">next-statement</span></pre>
:::
...
:::: {.columns}
::: {.column width="50%"}
![](images/m5-if-else.svg){height="400px" fig-align="center" .fragment}
:::
::: {.column width="50%"}
::: {.fragment}
<pre class="code-pattern" style="font-size:1.1em"><span class="kw">if</span> <span class="cond">condition</span>:
   <span class="stmt">statement1</span>
   <span class="stmt">statement2</span>
   <span class="cmt">…</span>
<span class="kw">else</span>:
   <span style="color:#c0392b;font-weight:600">statement3</span>
   <span style="color:#c0392b;font-weight:600">statement4</span>
   <span class="cmt">…</span>
<span style="color:#5d6d7e">next-statement</span></pre>
:::
:::
...
:::: {.columns}
::: {.column width="50%"}
![](images/m5-if-elif-else.svg){height="400px" fig-align="center" .fragment}
:::
::: {.column width="50%"}
::: {.fragment}
<pre class="code-pattern" style="font-size:1.1em"><span class="kw">if</span> <span class="cond">condition1</span>:
   <span class="cond">statement1</span>
<span class="kw">elif</span> <span style="color:#c0392b;font-weight:600">condition2</span>:
   <span style="color:#c0392b;font-weight:600">statement2</span>
<span class="kw">elif</span> <span style="color:#6c3483;font-weight:600">condition3</span>:
   <span style="color:#6c3483;font-weight:600">statement3</span>
<span class="kw">else</span>:
   <span class="stmt">default</span>
<span style="color:#5d6d7e">next-statement</span></pre>
:::
:::
::::
...
## `if` แยกกัน vs `if-elif`: ต่างกันมาก! {#m5-if-vs-elif .smaller}
...
![](images/m5-if-separate-trace.svg){width="330px" fig-align="center"}
...
![](images/m5-if-elif-trace.svg){width="330px" fig-align="center"}
...
:::: {.columns}
::: {.column width="50%"}
![](images/m5-nested-if.svg){height="400px" fig-align="center"}
:::
::: {.column width="50%"}
<pre class="code-pattern" style="font-size:1.1em"><span class="kw">if</span> <span class="cond">Expr1</span>:
   <span class="stmt">statement1</span>
<span style="display:inline-block; border:2px dashed #c0392b; border-radius:6px; padding:0.1em 0.5em; margin:0.15em 0">   <span class="kw">if</span> <span class="cond">Expr2</span>:
      <span class="stmt">statement2</span>
   <span class="kw">else</span>:
      <span class="stmt">statement3</span></span>
<span class="kw">else</span>:
   <span style="color:#c0392b;font-weight:600">statement4</span>
<span style="color:#5d6d7e">next-statement</span></pre>

[⬑ `if` ที่ซ้อนอยู่ด้านใน]{style="color:#c0392b; font-size:0.8em"}
:::
::::
...
![](images/m5-slice-studentid.svg){width="820px" fig-align="center"}
...
::: {.callout-note}
ต่อ Module 6: [Iterations — while-loop](module06.html) 👉
:::
