...
::: {.callout-important}
`not` · `and` · `or` **ไม่ได้อยู่ระดับเดียวกัน** — `not` แน่นสุด แล้ว `and` แล้วจึง `or`

`True or False and False` → `and` ทำก่อน → ได้ `True` (ไม่ใช่ `False`)
:::

## เห็นภาพ: `20 + 10 * 15 / 5` ทำงานทีละขั้น {#m3-precedence-trace .center}
